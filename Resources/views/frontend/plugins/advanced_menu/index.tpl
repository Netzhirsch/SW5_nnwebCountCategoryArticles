	...
	{foreach $categories as $category}
		...
		<li class="menu--list-item item--level-{$level}"{if $level === 0} style="width: 100%"{/if}>
			{block name="frontend_plugins_advanced_menu_list_item"}
				<a href="{$categoryLink|escapeHtml}" class="menu--list-item-link" title="{$category.name|escape}"{if $category.external && $category.externalTarget} target="{$category.externalTarget}"{/if}>
					{$category.name}
					{if is_numeric($nnwebCountCategoryArticlesCounts[$category.id]) && empty($nnwebCountCategoryArticlesHideZero[$category.id])}
...
{block name="frontend_plugins_advanced_menu"}
	{foreach $sAdvancedMenu as $mainCategory}
		{if !$mainCategory.active || $mainCategory.hideTop}
			{continue}
		{/if}
		{if $nnwebCountCategoryArticlesHideEmpty[$mainCategory.id]}
			{continue}
		{/if}
